function plotComplejo(lim_ejes,lim_ejes_modulo,lim_ejes_fase, eje_x, titulo, tam_letra, col, grosor, n, x)
    ...
    axis(lim_ejes_modulo); grid on; % L ??mites de los ejes. Grilla.
    set(gca,'FontSize', tam_letra); % Tama~no de letra para la leyenda y ejes.
    title("Modulo"); % Coloco t ??tulo para el gr ?afico.

    subplot(2,2,4);
    plot(n, angle(x), col, 'Linewidth', grosor);
    axis(lim_ejes_fase); 
    grid on; % L ??mites de los ejes. Grilla.
    set(gca,'FontSize', tam_letra); % Tama~no de letra para la leyenda y ejes.
    title("Fase"); % Coloco t ??tulo para el gr ?afico.
end
