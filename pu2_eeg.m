...
EEG_f=fft(eeg_f);
stemCompleto([-fs/2 fs/2 abs(min(EEG_f)) 6.5e4],'f [Hz]', '', '|EEG_f (IIR)|', 20, 'b.-', 1, f, abs(fftshift(EEG_f)));
%% RITMO ALFA
% e_eeg_f=eeg_f.^2;
% plotCompleto([0 t(end) 0 max(e_eeg_f)+5], 't[s]', 'Amplitud', 'Energía del encefalograma', 20, 'r-', 1, t, e_eeg_f);
% 
% [E_Y,E_Y_s]=fft_kit(eeg_f.^2,fs);
% stemCompleto([-fs/2 fs/2 abs(min(E_Y)) abs(max(E_Y))],'f [Hz]', '', '|E_Y|', 20, 'b.-', 1, f, abs(E_Y_s));
e_y=decimate(eeg_f.^2,16);
