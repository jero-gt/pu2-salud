...
stemCompleto([-fs/2 fs/2 abs(min(ECG_fLP_s)) 0.2],'f [Hz]', '', '|ECG_{fLP}|', 20, 'b.-', 1, f_cont, abs(ECG_fLP_s));

%plotCompleto([0 max(t2) min(ecg_contaminada) 7.5e-4], 't [s]', '', 'ecg contaminado', 20, 'r', 1.5, t2, ecg_contaminada);
plotStem([0 max(t2) min(ecg_contaminada) 7.5e-4], [-fs/2 fs/2 abs(min(ECG_s)) 0.2], 't [s]', 'Amplitud', 'f [Hz]', 'Amplitud', "ECG filtrada", "Escpectro ECG filtrado", 20, 'b.-', 1.5, t2, ecg_fLP, f_cont, abs(ECG_fLP_s))
