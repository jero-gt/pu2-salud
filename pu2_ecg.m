...
stemCompleto([-fs/2 fs/2 abs(min(ECG_s)) 0.2],'f [Hz]', 'Amplitud', '|ECG_{cont}|', 20, 'b.-', 1, f_cont, abs(ECG_s));

%% FILTRO PASABAJOS
load('filtroLP_100');
ecg_fLP = filter(LP, ecg_contaminada);
[ECG_fLP_s, ~] = fft_kit(ecg_fLP,fs);
stemCompleto([-fs/2 fs/2 abs(min(ECG_fLP_s)) 0.2],'f [Hz]', 'Amplitud', '|EEG_{fLP}|', 20, 'b.-', 1, f_cont, abs(ECG_fLP_s));

%% FILTRO RECHAZABANDA
load('filtroBS_49-53');
ecg_fBS = filter(BS, ecg_fLP);
[ECG_fBS_s, ~] = fft_kit(ecg_fBS,fs);
stemCompleto([-fs/2 fs/2 abs(min(ECG_fBS_s)) 0.2],'f [Hz]', 'Amplitud', '|EEG_fBP|', 20, 'b.-', 1, f_cont, abs(ECG_fBS_s));
